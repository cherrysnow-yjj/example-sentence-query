...
class QuickFind(QDialog):
    ...
    def _snt_template(self, keyword, sent, words):
        ...
        for word, tag in words:
            ...
            if a <= i and b >= j:
                ...
                word1 = usent[i:b]
                word2 = usent[b:j]
                wordstr = "<span style=\"background-color:#ccc\">%s</span>%s" % (
                    word1, word2)
            elif a > i and a < j and b >= j:
                word1 = usent[i:a]
                word2 = usent[a:j]
                wordstr = "%s<span style=\"background-color:#ccc\">%s</span>" % (
                    word1, word2)
            elif a > i and b < j:
                word1 = usent[i:a]
                word2 = usent[a:b]
                word3 = usent[b:j]
                wordstr = "%s<span style=\"background-color:#ccc\">%s</span>%s" % (
                    word1, word2, word3)
            else:
                wordstr = word
    ...
    def _tag_template(self, keytag, words):
        ...
        for word, tag in words:
            if tag == keytag:
                wordstr = "%s(<span style=\"background-color:#ccc;font-weight:600\">%s</span>) " % (
                    word, tag)
            else:
    ...
    def query(self):
        ...
        if len(keyword) == 0 or len(keyword.split()) > 1:
            QMessageBox.about(self, _translate("QuickFind", "出错了", None),
